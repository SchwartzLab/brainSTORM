...
# txBedAnnotation <- "/home/labs/schwartzlab/miguelg/BIGDATA/gene_annotations/p_occultum/pAbys_txome_rRNA.bed"
...
rdsFiles <- parallel::mclapply(mc.cores = nCores, META$BAM, function(file){
    bam2TxDT(BAMfile = file, 
             geneAnnot = TXOME, 
             genome = GENOME, 
             dtType = "covNuc", 
             outDir = "bam_Pa", 
...
# Lib Complexity 
META <- addBAMFileNames(META, outDir = "bam_Pa")
libComplexReport(META, maxExtrapolation = 2.01e6, steps = 1e5, verbose = T)
ggLCE <- gg_lce(META, tab_name = "bam_Pa/libCompReport.txt", speciesName = "P. abyssi")

# Nucleotide frequency
nucF_Pa <- fastq_nucFreq(META, nCores,firstN =  1e5)
ggNucF_Pa <- gg_nucFreq(nucF_Pa, "P. abyssi")
plot(ggNucF_Pa)

# Alignment efficiency report
f_tab_Pa <- files_table(META, "bam_Pa")
rReport_Pa <- reads_report(f_tab_Pa, META, nCores)
ggReadStats_Pa <- gg_readStats(rReport_Pa, "P. abyssi")
